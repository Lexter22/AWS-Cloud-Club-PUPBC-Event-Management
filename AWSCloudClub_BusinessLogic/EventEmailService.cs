...
namespace AWSCloudClub_BusinessLogic
{
    public class EventEmailService
    {
        ...
        public async System.Threading.Tasks.Task SendEmailAsync(string recipientEmail, Events ev, System.Threading.CancellationToken cancellationToken = default)
        {
            ...
            var username = _configuration["EmailSettings:SmtpUsername"] ?? _configuration["EmailSettings:Username"] ?? string.Empty;
            var password = _configuration["EmailSettings:SmtpPassword"] ?? _configuration["EmailSettings:Password"] ?? string.Empty;
            var enableTlsString = _configuration["EmailSettings:EnableTls"] ?? _configuration["EmailSettings:UseTls"] ?? "true";
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Failed to send event email (async): {ex}");
            }
        }
        ...
        {
            var configuredRecipient = _configuration["EmailSettings:ToAddress"] ?? _configuration["EmailSettings:ToEmail"] ?? string.Empty;
